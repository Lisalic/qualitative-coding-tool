...
from sqlalchemy import and_, delete, exists, func, insert, literal, null, or_, select, union_all
...
async def bulk_insert_coding_entries(
    ...
    """Insert ``entries`` (dicts with ``post_id``, ``code``, ``quote``,
    ``start_offset``, ``end_offset``, and optionally ``row_type``/``notes``
...
async def replace_entries_for_items(session: AsyncSession, file_id: int, items: list[dict]) -> None:
    """Delete then re-insert the ``coding_entries`` rows for exactly the
    given ``(row_type, post_id)`` keys -- the primitive behind both an AI
    recode (``coding_service.start_recode_items_job``) and a manual table
    edit (``coding_service.save_coding_rows``): both replace a chosen set
    of items' coding wholesale rather than diffing individual codes.

    ``items`` is ``[{"row_type", "post_id", "entries": [{"code", "quote",
    "start_offset", "end_offset", "notes"}]}]`` -- one entry per quote. An
    item with an empty ``entries`` list still has its old codes deleted
    and correctly ends up with zero codes (e.g. the AI decided no code
    applies, or a user cleared every code from a row) -- it is not left
    untouched.
    """
    if not items:
        return
    ...
    delete_condition = or_(*[and_(CodingEntry.row_type == rt, CodingEntry.post_id == pid) for rt, pid in keys])
    await session.execute(delete(CodingEntry).where(CodingEntry.file_id == file_id, delete_condition))
...
async def copy_entries(session: AsyncSession, *, source_file_id: int, target_file_id: int) -> int:
    """Copy every ``coding_entries`` row from ``source_file_id`` to
    ``target_file_id`` verbatim -- the ``coding_entries`` half of
    duplicating a whole coding artifact
    (``coding_service.duplicate_coding`` also copies the codebook
    snapshot via ``artifact_content_repo`` and the submissions/comments
    rows via ``raw_data_repo.copy_all_rows``). Same insert-from-select
    shape as ``raw_data_repo.copy_rows_by_id``/``copy_all_rows``.
    """
    # Exclude both the surrogate ``id`` (a fresh one per copied row -- the
    # source's ids are already taken) and ``file_id`` (set explicitly to
    # the target below) from the copied column list.
    non_id_cols = [c for c in CodingEntry.__table__.c if c.name not in ("id", "file_id")]
    col_names = ["file_id"] + [c.name for c in non_id_cols]
    count_result = await session.execute(
        select(func.count()).select_from(CodingEntry).where(CodingEntry.file_id == source_file_id)
    )
    n = count_result.scalar() or 0
    if n:
        src_select = select(literal(target_file_id).label("file_id"), *non_id_cols).where(
            CodingEntry.file_id == source_file_id
        )
...
async def code_summary_with_samples(
    ...
    for code, count in freq:
        result = await session.execute(
            select(CodingEntry.quote)
            .where(CodingEntry.file_id == file_id, CodingEntry.code == code)
...
async def list_rows_with_codes(
    ...
    *,
    ...
    entries = (
        await session.execute(select(CodingEntry).where(CodingEntry.file_id == file_id, entries_condition))
    ).scalars().all()
